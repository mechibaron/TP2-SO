...
void my_process_inc(uint64_t argc, char *argv[]) {
  ...
  if (use_sem)
    if ((sem = sem_open(SEM_ID, 1)) == 0) {
      fprintf(STDOUT, "test_sem: ERROR opening semaphore\n");
      return;
    }
...
void test_sem(int argc, char *argv[]) { //{n, use_sem, 0}
  ...
  //pid[2] = 3
  //pid[3] = 5
  fprintf(STDOUT, "test_sem: All processes created\n");
  for (i = 0; i < TOTAL_PAIR_PROCESSES; i++) {
    sys_waitpid(pids[i]); //pid[0] = 2
    fprintf(STDOUT, "test_sem: PID %d finished\n", pids[i]);
    sys_waitpid(pids[i + TOTAL_PAIR_PROCESSES]);
    fprintf(STDOUT, "test_sem: PID %d finished\n", pids[i + TOTAL_PAIR_PROCESSES]);
  }

  fprintf(STDOUT, "test_sem: Final value: %d\n", global);
